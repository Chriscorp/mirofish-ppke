...
from ..utils.id_validator import validate_safe_id, safe_join
...
# Version enrichie : demande à l'agent de parler comme un humain, pas comme une IA
INTERVIEW_PROMPT_PREFIX = (
    "你是一个拥有独特背景、价值观和社交媒体历史的真实人物。"
    "请结合你的完整人设描述、你的过往记忆与行动历史,"
    "以第一人称、自然口语化的方式直接回答下面的问题。"
    "不要调用任何工具,不要解释推理过程,"
    '不要以"作为AI"开头,直接表达你真实的想法、情绪和立场:'
)
    ...
    try:
        ...
        def run_prepare():
            ...
            try:
                ...
                # Sauvegarder les mémoires des agents (persistance cross-simulation)
                save_agent_memories(simulation_id)
...
# ============== Agent Memory (persistance cross-simulation) ==============

def _get_simulation_dir(simulation_id: str) -> str:
    """Retourne le chemin du dossier de simulation."""
    from ..config import Config
...
def _get_memory_dir(simulation_id: str) -> str:
    """Retourne le chemin du dossier de mémoire pour une simulation."""
    sim_dir = _get_simulation_dir(simulation_id)
    memory_dir = os.path.join(sim_dir, "memory")
    return memory_dir


# ── Persistance des conversations (chat, interview, survey) ───────────────────

CONVERSATIONS_FILE = "conversations.jsonl"


def _save_conversation(simulation_id: str, entry: dict) -> bool:
    """
    Sauvegarde une conversation (chat, interview, survey) dans un fichier JSONL
    persistant. Le fichier est stocké dans le dossier de simulation.
    """
    try:
        sim_dir = _get_simulation_dir(simulation_id)
        conv_path = os.path.join(sim_dir, CONVERSATIONS_FILE)
        entry["_saved_at"] = datetime.now().isoformat()
        with open(conv_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps(entry, ensure_ascii=False) + '\n')
        return True
    except Exception as e:
        logger.warning(f"Save conversation failed: {e}")
        return False


def _load_conversation_history(simulation_id: str, conv_type: str = None,
                                agent_id: int = None, limit: int = 100) -> list:
    """
    Charge l'historique des conversations depuis le fichier JSONL.
    Peut être filtré par type (chat|interview|survey) et par agent_id.
    ...
    try:
        ...
        with open(conv_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    if conv_type and entry.get("type") != conv_type:
                        continue
        ...
        logger.warning(f"Load conversations failed: {e}")
        return []


@simulation_bp.route('/conversations', methods=['POST'])
def get_conversation_history():
    """
    Récupère l'historique persistant des conversations.
    Retourne les entrées du fichier conversations.jsonl.
    """
    try:
        data = request.get_json() or {}
        simulation_id = data.get('simulation_id')
        conv_type = data.get('type')  # chat | interview | survey
...
def save_agent_memories(simulation_id: str) -> bool:
    """
    Sauvegarde les profils des agents de la simulation dans un dossier mémoire
    pour pouvoir les réutiliser lors d'un run ultérieur.

    Copie reddit_profiles.json → memory/agent_memories.json
    """
    try:
        sim_dir = _get_simulation_dir(simulation_id)
        profiles_path = os.path.join(sim_dir, "reddit_profiles.json")
        if not os.path.exists(profiles_path):
            logger.warning(f"Agent memory save: aucun profil trouvé pour {simulation_id}")
            return False

        memory_dir = _get_memory_dir(simulation_id)
        os.makedirs(memory_dir, exist_ok=True)

        import shutil
        shutil.copy2(profiles_path, os.path.join(memory_dir, "agent_memories.json"))

        # Compter le nombre d agents
        with open(profiles_path, 'r', encoding='utf-8') as f:
            profiles = json.load(f)
        agent_count = len(profiles) if isinstance(profiles, list) else 0

        logger.info(f"Agent memories saved: {simulation_id} ({agent_count} agents)")
        return True
    except Exception as e:
        logger.error(f"Save agent memories failed: {e}")
        return False


def load_agent_memories(simulation_id: str) -> Optional[List[Dict[str, Any]]]:
    """
    Charge les mémoires d'agents sauvegardées pour une simulation.
    Retourne la liste des profils, ou None si aucune mémoire n'existe.
    """
    try:
        memory_file = os.path.join(_get_memory_dir(simulation_id), "agent_memories.json")
        ...
        agent_count = len(profiles) if isinstance(profiles, list) else 0
        logger.info(f"Agent memories loaded: {simulation_id} ({agent_count} agents)")
        return profiles
    except Exception as e:
        logger.error(f"Load agent memories failed: {e}")
        return None


def agent_memories_available(simulation_id: str) -> Dict[str, Any]:
    """
    Vérifie si des mémoires d'agents sont disponibles pour une simulation.
    """
    try:
        memories = load_agent_memories(simulation_id)
        if memories is None:
            return {"available": False, "agent_count": 0}
        return {"available": True, "agent_count": len(memories) if isinstance(memories, list) else 0}
    except Exception:
        return {"available": False, "agent_count": 0}


@simulation_bp.route('/memories/<simulation_id>', methods=['GET'])
def check_agent_memories(simulation_id: str):
    """API: Vérifie si des mémoires d'agents existent pour une simulation."""
    result = agent_memories_available(simulation_id)
...
def start_simulation():
    ...
    try:
        ...
        load_previous_memory = data.get('load_previous_memory', False)  # 可选:repartir des agents du run précédent
        ...
        # Agent memory persistence: load previous profiles if requested
        memories_loaded = False
        if load_previous_memory:
            logger.info(f"Load previous agent memories: {simulation_id}")
            memories = load_agent_memories(simulation_id)
            if memories:
                # Copy memories to the profiles file that the simulation engine expects
                sim_dir = _get_simulation_dir(simulation_id)
                profiles_path = os.path.join(sim_dir, "reddit_profiles.json")
                with open(profiles_path, 'w', encoding='utf-8') as f:
                    json.dump(memories, f, ensure_ascii=False, indent=2)
                logger.info(f"Previous agent memories loaded: {len(memories)} agents")
                memories_loaded = True
            else:
                logger.warning(f"No previous agent memories found for {simulation_id}")
...
# Fallback LLM interview: utilise l'LLM directement quand l'environnement OASIS n'est plus en vie
def _interview_fallback_llm(simulation_id: str, agent_id: int, prompt: str, platform: str = "twitter") -> dict:
    """
    Interviewe un agent via LLM direct (fallback quand l'env OASIS est arrêté).
    Utilise le profil sauvegardé de l'agent comme contexte.
    """
    try:
        # Charger les profils
        sim_dir = _get_simulation_dir(simulation_id)
        ...
        for p in profiles:
            if isinstance(p, dict):
                if p.get("user_id") == agent_id or p.get("agent_id") == agent_id:
                    agent_profile = p
                    agent_username = p.get("username", p.get("name", str(agent_id)))
                    break

        if not agent_profile:
            return {"success": False, "error": f"Agent {agent_id} not found in profiles"}

        # Construire le prompt avec le contexte de l'agent
        persona = agent_profile.get("persona", "")
        bio = agent_profile.get("bio", "")
        name = agent_profile.get("name", agent_username)
        profession = agent_profile.get("profession", "")

        system_prompt = (
            f"Tu es {name}, un(e) gérant(e) de magasin Netto en Provence.\n"
            f"Profil: {persona}\n"
            f"Bio: {bio}\n"
            f"Profession: {profession}\n\n"
            f"Réponds de façon naturelle, en français, comme le ferait un gérant de supermarché. "
            f"Sois concis (2-3 phrases max). Ne mentionne pas que tu es une IA. "
            f"Exprime ton opinion personnelle basée sur ton profil."
        )
        ...
        _save_conversation(simulation_id, {
            "type": "interview",
            "agent_id": agent_id,
            "agent_name": name,
            "prompt": prompt,
        ...
        logger.warning(f"Interview LLM fallback failed for agent {agent_id}: {e}")
...
def interview_agent():
    ...
    try:
        ...
        if env_alive:
            # Environnement OASIS en vie : interview normale
            optimized_prompt = optimize_interview_prompt(prompt)
            ...
            # Fallback LLM : simule l agent via LLM avec son profil
            logger.info(f"Env not alive, using LLM fallback for agent {agent_id}")
            result = _interview_fallback_llm(
                simulation_id=simulation_id,
                agent_id=agent_id,
                prompt=prompt,
            ...
            if result.get("success"):
                ...
                return jsonify({
                    "success": False,
                    "error": result.get("error", t('api.envNotRunning'))
                }), 400
...
def interview_agents_batch():
    ...
    try:
        ...
        if env_alive:
            ...
            if result.get("success"):
                results_data = result.get("results", result.get("result", {}))
                all_results = results_data.get("results", results_data) if isinstance(results_data, dict) else {}
                for key, entry in all_results.items():
                    if isinstance(entry, dict) and entry.get("agent_id") is not None:
                        _save_conversation(simulation_id, {
                            "type": "survey",
                            "agent_id": entry["agent_id"],
                            "agent_name": entry.get("agent_name", str(entry["agent_id"])),
                            "prompt": entry.get("prompt", prompt),
                            "response": entry.get("response", ""),
                            "platform": entry.get("platform", platform or "parallel"),
                            "fallback": False,
            ...
            # Fallback LLM pour chaque agent
            logger.info(f"Env not alive, using LLM fallback for batch interview ({len(interviews)} agents)")
            results = {}
            for interview in interviews:
                aid = interview.get('agent_id')
                iprompt = interview.get('prompt', '')
                iplatform = interview.get('platform', platform or 'twitter')
                if aid is None:
                    continue
                r = _interview_fallback_llm(
                    simulation_id=simulation_id,
                    agent_id=aid,
                    prompt=iprompt,
                    platform=iplatform
                ...
                if r.get("success"):
                    key = f"{iplatform}_{aid}"
                    results[key] = {
                        "agent_id": aid,
                        "response": r.get("response", ""),
                        "platform": iplatform,
                        "agent_name": r.get("agent_name", str(aid)),
                        "fallback": True
                    }
            return jsonify({
                "success": True,
...
def interview_all_agents():
    ...
    try:
        ...
        if env_alive:
            ...
            # Fallback non disponible en mode "all" (on ne connaît pas les agent_ids)
            logger.info(f"Env not alive for interview_all, cannot use LLM fallback (no agent_ids)")
            return jsonify({
                "success": False,
                "error": "L'environnement de simulation est arrêté. Utilisez l'interface de discussion avec un agent spécifique, ou redémarrez la simulation."
            }), 400
